...
input_file = args[1]
output_file = args[2]
#output_file <- gsub("gencode.v27.bed", "gene_coordinates_gencode.v27.bed", output_file)



# input_file = "data/gencode/gencode.v27.basic.annotation.gtf"
# coding = "data/gencode/gencode_protein_coding_basic.gtf"
# output_file = "data/gencode/promoter_coordinates_gencode.v27.bed"

# Read GTF file
#gtf <- readGFF("gencode_protein_coding_basic.gtf")
command <- paste("grep 'protein_coding'",input_file, ">",'coding')
system(command)
gtf <- readGFF('coding')
command <- "rm coding"
system(command)
...
gtf <- gtf[gtf$type %in% c("gene","transcript") & gtf$gene_type %in% c("protein_coding") & !(gtf$seqid %in% c('chrM')),!(colnames(gtf) %in% c("score","phase"))]
...
for (i in 1:length(gene_dict$gene_id)) {
  ...
  query <- reduce(IRanges(tmp$start, end = tmp$end))
